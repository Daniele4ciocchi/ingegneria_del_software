#include "specializzazione_medico.h"

SpecializzazioneMedico::SpecializzazioneMedico(char* medico_id, char* specializzazione_nome) {
...
SpecializzazioneMedico* SpecializzazioneMedico::from_stream(redisReply* reply, int stream_num, int msg_num) {
    ...
    if (read_fields != 0b11) {
        throw std::invalid_argument("Missing fields in SpecializzazioneMedico");
    }

    return new SpecializzazioneMedico(medico_id, specializzazione_nome);
}