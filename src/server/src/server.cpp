...
void Server::add_new_clients() {
    ...
    do {
        ...
        sprintf(query, "INSERT INTO client(server_name, file_descriptor, iconnection) VALUES (\'%s\', %d, CURRENT_TIMESTAMP)", server, new_client);
...
void Server::send_response(int client_id, std::string out_str) {
    ...
    sprintf(query, "WITH max_client_conn AS (SELECT max(iconnection) AS instant FROM client WHERE server_name = \'%s\' AND file_descriptor = %d), "
                    "     last_request AS (SELECT MAX(c.irequest) AS instant FROM communication AS c, max_client_conn AS m WHERE c.client_server_name = \'%s\' AND c.client_file_descriptor = %d AND c.client_iconnection = m.instant) "
                    "UPDATE communication SET response_status = \'%s\', iresponse = CURRENT_TIMESTAMP "
                    "WHERE client_server_name = \'%s\' AND client_file_descriptor = %d AND client_iconnection = (SELECT instant FROM max_client_conn) AND irequest= (SELECT instant FROM last_request)", server, client_id, server, client_id, first_line.c_str(), server, client_id);
    query_res = log_db.RunQuery(query, false);
...
void Server::receive(int i) {
    ...
    sprintf(query, "INSERT INTO communication(request, irequest, client_server_name, client_file_descriptor, client_iconnection)"
                   "VALUES (\'%s\', CURRENT_TIMESTAMP, \'%s\', %d, (SELECT iconnection FROM Client WHERE idisconnection IS NULL and server_name = \'%s\' and file_descriptor = %d))", msg.c_str(), server, i, server, i);
